...
export interface IEmailValidationModel extends Model<IEmailValidationDoc> {
    ...
    migrateEmailValidationUserId(
        temporaryUserId: Types.ObjectId, userId: Types.ObjectId
    ): Promise<IEmailValidation | null>;
...
EmailValidationSchema.statics.migrateEmailValidationUserId = async function (temporaryUserId, userId) {
    logger.debug(`[migrateEmailValidationUserId] migrate emailvalidation ${temporaryUserId} => ${userId}`);
    const data = {
        userId
    };
    return EmailValidation.findOneAndUpdate({ userId: temporaryUserId }, data, { new: true });
};
