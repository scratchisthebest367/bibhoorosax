...
export interface IUserDoc extends IUser, PassportLocalDocument<Types.ObjectId>, MongooseTimestamps {
    ...
    upgradeToFullUser(): Promise<IUserDoc | false>;
...
export interface IUserModel extends Model<IUserDoc> {
    ...
    createTemporaryUser(): Promise<IUserDoc>;
    ...
    migrateTemporaryUser(temporaryUser: IUserDoc, user: IUserDoc): Promise<IUserDoc | null>;
...
UserSchema.statics.createUser = async function (user = {}) {
    ...
    const { id, temporary } = dbUser;
    const log = { userId: id, temporary };
...
UserSchema.statics.createTemporaryUser = function () {
...
UserSchema.statics.addEmailAuthProvider = async function (validation) {
    ...
    if (user.temporary && existingUser && validation.email === existingUser.email) {
        logger.debug(
            `[addEmailAuthProvider] an existing user with the same primary email was discovered existingUserId: ${existingUser.id}`
        );
        await User.migrateTemporaryUser(user, existingUser);
        const data = {
        ...
        const updatedUser = await User.findOneAndUpdate({ _id: existingUser.id }, data, { new: true });
...
UserSchema.statics.migrateTemporaryUser = async function (temporaryUser, user) {
    logger.debug(`[migrateTemporaryUser] migrate temporary user ${temporaryUser.id} => ${user.id}`);
    const validation = await EmailValidation.migrateEmailValidationUserId(temporaryUser._id!, user._id!);
    if (validation) {
        const validationUser = await User.findOne({ _id: validation.userId });
        await validationUser?.setEmailVerificationStatus('pending', validation);
    }
    temporaryUser.connections.forEach((connection) => {
        if (!_.find(user.connections, { type: connection.type })) {
            user.connections.push(connection);
        }
    });
    await user.save();
    await Project.migrateProjectsToOwner(temporaryUser._id!, user._id!);
    await User.deleteUserById(temporaryUser._id!);
...
UserSchema.methods.upgradeToFullUser = async function () {
    if (!this.temporary) {
        return false;
    }
