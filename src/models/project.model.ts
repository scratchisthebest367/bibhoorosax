...
export interface IProjectModel extends Model<IProjectDoc>, SoftDeleteModel<IProjectDoc> {
    ...
    findProjectByIdAndUser(projectId: Types.ObjectId, user: IUserDoc, permission: Permission): Promise<IProjectDoc | null>;
    findProjectByIdAndUserRoles(projectId: Types.ObjectId, user: IUserDoc, roles: CollaboratorRole[]): Promise<IProjectDoc | null>;
    ...
    findProjectByIdAndCollaboratorToken(id: Types.ObjectId, token: string): Promise<IProjectDoc | null>;
    ...
    addInvitedCollaborator(project: IProjectDoc, user: IUserDoc, invite: ICollaborator): Promise<IProjectDoc | null>;
    updateCollaboratorByTokenAndUserId(project: IProjectDoc, inviteToken: string, userId: Types.ObjectId): Promise<IProjectDoc | null>;
    removeCollaboratorById(project: IProjectDoc, collaboratorId: Types.ObjectId): Promise<IProjectDoc | null>;
    updateCollaboratorById(project: IProjectDoc, collaboratorId: Types.ObjectId, update: any): Promise<IProjectDoc | null>;
...
ProjectSchema.statics.cancelSubscription = async function (projectId, { immediate, skipEmail } = {}) {
    ...
    const reportAnalytics = (tierId: string, owner: IUserDoc | null) => {
        if (customerTierService.isTrialTier(tierId)) {
            analytics.track('Trial Expired', { projectId, tierId }, owner);
        } else {
            analytics.track('Subscription Canceled', { projectId, tierId }, owner);
        }
    };
    if (immediate) {
        ...
        reportAnalytics(previousTierId, owner);
        ...
        const owner = await User.findById(project.ownerId);
        reportAnalytics(project.subscription.safeTierId, owner);
        if (!skipEmail) {
