...
module.exports = {
    temp: async (req, res, next) => {
        ...
        if (user) {
            return res.status(403).json(ResponseErrors.RegisterLoggedInError);
        }
        try {
            user = await User.createTemporaryUser();
            req.login(user, (err) => {
        ...
        if (userGroup && user.temporary) {
            // switch user group if provided and user is temporary (anonymous)
    ...
    register: async (req, res, next) => {
        ...
        logger.debug(`[register] req.temporaryUser: ${req.temporaryUser}`);
        ...
        try {
            ...
            // Check if a *logged in* user already has an email authProvider.
            if (_.get(user, 'authProviders.email.providerUserId')) {
                throw ResponseErrors.RegisterLoggedInError;
            }

            // Registration
            if (!user) {
                logger.debug('[register] create temporary user...');
                user = await User.createTemporaryUser();
            }
    ...
    login: (req, res) => {
        ...
        if (req.temporaryUser) {
            return User.migrateTemporaryUser(req.temporaryUser, req.user).then(user => {
                analytics.identify(user, req);
