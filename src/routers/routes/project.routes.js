...
module.exports = {
        ...
        try {
            ...
            const collaboratorsResponse = await Promise.all(
                collaborators.map(async (collaborator) => {
                    ...
                    const result = {
                        id: collaborator.id,
                        userId: _.get(collaborator, 'userId', '').toString(),
                        email,
                        role
                    };
    ...
    inviteCollaborator: async (req, res, next) => {
        try {
            ...
            if (!project.checkTierAllowanceForFeature('collaborators', { role })) {
                throw new errorUtils.ResponseError('ProjectTierExceeded');
            }
            ...
            logger.error('[inviteCollaborator] failed', { error: e, userId: req.user.id });
    ...
    removeCollaborator: (req, res, next) => {
        ...
        return Project.findProjectByIdAndUser(projectId, user, CollaboratorRole.Permission.MANAGE_COLLABORATORS)
            .then((project) => {
                if (!project) {
                    throw ResponseErrors.NotFound;
                }
                return Project.removeCollaboratorById(project, collaboratorId).then((project) => {
                    return res.status(200).json(project);
                });
            })
            .catch(next);
    },
    async updateCollaborator(req, res, next) {
        const user = req.user;
        const projectId = req.params.id;
        ...
        try {
            ...
            if (!collaborator) {
                throw ResponseErrors.CollaboratorDoesNotExist;
            }
            ...
            logger.error('[updateCollaborator] failed', { error: err, userId: req.user.id });
            return res.status(err.status || 500).json({ message: err.message });
        }
    },
    makeAction: (req, res) => {
        ...
        if (!permission) {
            const error = ResponseErrors.ActionError('No permissions for action');
        ...
        return Project.findProjectByIdAndUser(projectId, req.user, permission)
            ...
            .catch((err) => {
                ...
                const error = isDescribedError ? err.data : ResponseErrors.ActionError(err.message || _.startCase(`${action} failed`));
    ...
    checkCollaboratorToken: (req, res, next) => {
        const projectId = req.params.id;
        const token = _.get(req.query, 'token');
        if (!token) {
            throw ResponseErrors.CollaboratorTokenNotProvided;
        }
        return Project.findProjectByIdAndCollaboratorToken(projectId, token)
            .then((project) => {
                if (!project) {
                    throw ResponseErrors.CollaboratorTokenInvalid;
                }
                return project;
            })
            .then((project) => {
    ...
    acceptInvite: async (req, res, next) => {
        try {
            ...
            const token = _.get(req.query, 'token');
            if (!token) {
                throw new errorUtils.ResponseError('CollaboratorTokenNotProvided');
            }

            let project = await Project.findProjectByIdAndCollaboratorToken(projectId, token);
            if (!project) {
                throw new errorUtils.ResponseError('CollaboratorTokenInvalid');
            }
            ...
            if (userId === project.ownerId.toString()) {
                ...
                return res.status(403).json(customError);
            }

            // not supported flow anymore
            // fixme need cleanup
            if (project.collaborationInviteToken === token) {
                // user can be invited by direct URL like /project/${project._id}/accept-collaborator-invite?token=${project.collaborationInviteToken}
                // we have to simulate invite action to create DB record for invited user
                project = await Project.addInvitedCollaborator(project, user, { inviteEmail: userEmail, inviteToken: token });
            }
...
async function getCollaboratorEmail(collaborator) {
    if (!collaborator.userId) {
        ...
        const userEmail = _.get(collaboratorUser, 'email');
        if (userEmail) {
            return userEmail;
        }
        ...
        if (collaboratorUser && collaboratorUser.emailVerification === 'pending') {
            ...
            return _.get(userValidation, 'email');
        }
...
function getCollaboratorRole(project, collaborator) {
