...
import { loadCommonRequireMock, mockServerModules } from '../../../test-utils/requireMock';
...
describe('Collaborators', () => {
    ...
    describe('invite collaborators', () => {
        ...
        test('with invalid email', async () => {
            ...
            await inviteCollaborator(projectId, { email: '' }).expect('Content-Type', /json/).expect(500);
        });
        ...
        test('with invalid role', async () => {
            ...
            const response = await inviteCollaborator(projectId, inviteData).expect('Content-Type', /json/).expect(500);

            expect(response.body.message).toBe(`Role ${inviteData.role} is not valid`);
        });
        ...
        describe('by user', () => {
            ...
            test('with permissions to invite', async () => {
                ...
                jest.spyOn(Project, 'addInvitedCollaborator').mockImplementation((): any => Promise.resolve(project));
    ...
    describe('accept invite', () => {
        ...
        test('with invalid token', async () => {
            ...
            expect(noTokenRes?.body.status).toBe(403);

            expect(invalidTokenRes?.body.name).toBe('CollaboratorTokenInvalid');
            expect(invalidTokenRes?.body.status).toBe(403);

            expect(reusedToken?.body.name).toBe('CollaboratorTokenInvalid');
            expect(reusedToken?.body.status).toBe(403);
        });
        ...
        describe('by user', () => {
            ...
            beforeEach(async () => {
                ...
                jest.spyOn(Project, 'updateCollaboratorByTokenAndUserId').mockImplementation(jest.fn(() => Promise.resolve(project)));
            });
