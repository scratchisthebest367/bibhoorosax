...
export function temporaryUserMiddleware(req: Request, res: Response, next: NextFunction): void {
    // This will keep the temporary user referenced in the request even after the user has logged in to another user during the middleware stage
    if (req.user && req.user.temporary) {
        req.temporaryUser = req.user;
    }
    next();
}
