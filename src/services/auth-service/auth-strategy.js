...
const authStrategy = (strategyType, { mustAgreeTos = true, alwaysAllowConnectionOnly = false, forceConnectionOnly = false, addToUserGroup = false }, req, accessToken, refreshToken, profile, done) => {
    ...
    const registerFlow = async dbUser => {
        ...
        return Promise.resolve(dbUser)
            ...
            .then(dbUser => dbUser.temporary ? dbUser.upgradeToFullUser() : dbUser)
            .then(dbUser => {
    ...
    return Promise.all([User.findUserByProviderId(authName, profile.id), User.findUserByEmail(providerEmail)])
        .then(([dbProviderUser, dbEmailUser]) => {
            ...
            if (!req.user || req.temporaryUser) {
                if (dbStrategyUser) {
                    // User found updating provider + connection
                    if (req.temporaryUser) {
                        analytics.track('[AuthStrategy] Temporary user signed into an existing user', authLoggerObject, dbStrategyUser);
                        return User.migrateTemporaryUser(req.user, dbStrategyUser).then(addProviderAndConnection);
                    } else {
                ...
                if (req.temporaryUser) {
                    if (mustAgreeTos && !tosVersion && allowConnectionOnly) {
                        analytics.track('[AuthStrategy] Temporary user added connection without upgrading to full user', authLoggerObject, req.user);
                        return addProviderAndConnection(req.user, true);
                    }

                    analytics.track('[AuthStrategy] Temporary user upgraded to full user', authLoggerObject, req.user);
                    return registerFlow(req.user);
                } else {
